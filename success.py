...
melody1 = [12, 5, 9, 4, 7, 0] # these are note numbers; use None to create a rest
lengths1 = [2, 1, 1, 3, 1, 1] # these are note lengths in beats

# note numbers and note lengths for the second melody
melody2 = [0, 4, 7, 5, 9, 12] # these are note numbers; use None to create a rest
lengths2 = [1, 1, 2, 1, 1, 3] # these are note lengths in beats
...
for noteNum, noteLength in zip(melody2, lengths2):
   ...
   amp = 0.2
   pitch = getPitchFromScale(noteNum)
   # use the complex instrument with arguments to sound one note
   print('i "complex" ' + str(startTime) + ' ' + str(noteDuration) + ' ' + str(amp) + ' ' + str(pitch) + ' ' + str(attackDuration) + ' ' + str(decayDuration))
   startTime += noteDuration # advance to the time for the next note
